...
lora_init(void)
{
   ...
   //SPI ja iniciado no circuito ETH
   /*
   ...
   ESP_LOGI(TAG, "Lora Version: %x", version);
   ...
   //lora_write_reg(REG_MODEM_CONFIG_3, 0x04); //lora sync word não é o reg 0x26 no HopeRF, e não foi achado qual reg correto
   lora_set_tx_power(20);

   lora_set_spreading_factor(7);
   lora_set_bandwidth(125E3);
   
   //lora_set_sync_word(0x12); //lora sync word não é o reg 0x39 no HopeRF, e não foi achado qual reg correto

   lora_set_preamble_length(10); //lora set preamble length 10
/**
 * Configure explicit header mode.
 * Packet size will be included in the frame.
 */
